    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-slate-100 p-8">
      <div className="flex justify-between items-center mb-10 gap-4">
        <h1 className="text-3xl font-bold">
          Study<span className="text-yellow-400">Stack</span>
        </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-5">
          ...
          <p className="text-slate-400 text-sm mt-1">Study Groups</p>
        </div>
        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-5">
          <p className="text-3xl font-bold text-yellow-400">{deadlines.length}</p>
          <p className="text-slate-400 text-sm mt-1">Total Deadlines</p>
        </div>
        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-5">
          <p className="text-3xl font-bold text-yellow-400">{upcoming.length}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {groups.map((group) => (
                <div
                  ...
                  className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-6 flex flex-col gap-2 hover:border-yellow-400/30 hover:-translate-y-1 transition cursor-pointer"
                >
          ...
          <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-xl p-5 flex flex-col gap-3">
            {upcoming.length === 0 ? (
              ...
              upcoming.map((d) => (
                <div
                  key={d.id}
                  className="bg-white/5 rounded-lg p-4 flex justify-between items-center border border-white/5"
                >
                  <div>
