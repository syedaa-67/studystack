...
export const ThemeSwitcher = () => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      >
        {icons[mode]}
        <span className="text-xs text-gray-400 hidden sm:inline min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">{labels[mode]}</span>
        <ChevronDown size={14} className={'text-gray-400 transition-transform ' + (open ? 'rotate-180' : '')} />
      </button>

      {open && (
        <>
          <div className="fixed inset-0 z-40 min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto" onClick={() => setOpen(false)} />
          <div className="absolute right-0 mt-2 z-50 w-56 p-3 rounded-xl bg-gray-900/95 border border-white/10 shadow-2xl backdrop-blur-sm min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
            <div className="space-y-2 min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-4 gap-1 min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                {allModes.map((m) => {
                  const active = mode === m;
                  ...
                  return (
                    <button
                      ...
                      title={labels[m]}
                    >
                      <div className="text-lg min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">{icons[m]}</div>
                      <span className="text-[9px] font-medium min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">{labels[m]}</span>
                    </button>
                  );
                })}
              </div>
              <div className="pt-2 border-t border-white/5 text-center min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                <p className="text-[10px] text-gray-500 min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                  Current: <span className="text-gray-300 min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">{labels[mode]}</span>
                </p>
              </div>
            </div>
          </div>
