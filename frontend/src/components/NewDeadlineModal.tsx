    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="bg-slate-800/95 backdrop-blur-lg border border-white/10 text-slate-100 rounded-xl p-6 w-full max-w-md min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6 min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          <h3 className="text-lg font-bold min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">New Deadline</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          <div>
            <label className="text-xs text-slate-400 mb-1 block min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">Title</label>
            <input
              type="text"
              value={title}
          ...
          <div>
            <label className="text-xs text-slate-400 mb-1 block min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">Due Date</label>
            <input
              type="date"
              value={dueDate}
              onChange={(e) => setDueDate(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm text-slate-100 focus:outline-none focus:border-yellow-400/50 transition [color-scheme:dark] min-h-screen w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto"
            />
          </div>
